...
trait QueryImplicits extends Rogue {
  ...
  implicit def mandatoryFieldToSelectField[M <: BsonRecord[M], V](f: RequiredRecordField[V, M]): SelectField[V, M] =
    new MandatorySelectField(f)

  implicit def optionalFieldToSelectField[M <: BsonRecord[M], V](f: OptionalRecordField[V, M]): SelectField[Option[V], M] =
  ...
  implicit def rrequiredFieldToSelectField[M, V](
    f: RequiredField[V, M]): SelectField[V, M] = new MandatorySelectField(f)

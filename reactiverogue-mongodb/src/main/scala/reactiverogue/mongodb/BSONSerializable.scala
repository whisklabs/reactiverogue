...
object BSONSerializable {
  ...
  implicit object DateTimeIsBSONSerializable extends BSONSerializable[DateTime] {

    override def asBSONValue(v: DateTime): BSONValue =
      BSONDateTime(v.getMillis)

    override def fromBSONValue = {
      case BSONDateTime(v) => new DateTime(v)
    }
  }

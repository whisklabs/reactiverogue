...
object AddSelect {
  implicit def addSelect[Rest >: Ord with Lim with Sk with Or with Sh]
...
object AddLimit {
  implicit def addLimit[Rest >: Ord with Sel with Sk with Or with Sh]
...
object AddSkip {
  implicit def addSkip[Rest >: Ord with Sel with Lim with Or with Sh]
...
object AddOrClause {
  implicit def addOrClause[Rest >: Ord with Sel with Lim with Sk with Sh]
...
object AddShardAware {
  implicit def addShardAware[Rest >: Ord with Sel with Lim with Sk with Or]
    : AddShardAware[Rest with ShardKeyNotSpecified,
                    Rest with ShardKeySpecified,
                    Rest with AllShardsOk] = null
}
...
@implicitNotFound(
  msg = "${M} is a sharded collection. Either specify the shard key or use `.updateMulti()`.")

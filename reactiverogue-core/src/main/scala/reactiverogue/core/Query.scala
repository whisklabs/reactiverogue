...
case class Query[M, R, +State](
   ...
   * now Ordered.
   */
  def orderAsc[S2](field: M => AbstractQueryField[_, M])(implicit ev: AddOrder[State, S2]): Query[M, R, S2] =
    this.copy(order = Some(MongoOrder(List((field(meta).field.name, true)))))

  def orderDesc[S2](field: M => AbstractQueryField[_, M])(implicit ev: AddOrder[State, S2]): Query[M, R, S2] =
    this.copy(order = Some(MongoOrder(List((field(meta).field.name, false)))))

  def andAsc(field: M => AbstractQueryField[_, M])(implicit ev: State <:< Ordered): Query[M, R, State] =
    this.copy(order = Some(MongoOrder((field(meta).field.name, true) :: order.get.terms)))

  def andDesc(field: M => AbstractQueryField[_, M])(implicit ev: State <:< Ordered): Query[M, R, State] =
  ...
  def modify(clause: M => ModifyClause)(implicit ev1: Required[State, Unselected with Unlimited with Unskipped],
    ev2: ShardingOk[M, State]): ModifyQuery[M, State] = {
  ...
  def findAndModify[F](clause: M => ModifyClause)(implicit ev1: Required[State, Unlimited with Unskipped],
    ev2: RequireShardKey[M, State]): FindAndModifyQuery[M, R] = {
    FindAndModifyQuery[M, R](this, MongoModify(Nil)).findAndModify(clause)
  }

  def findAndModifyOpt[V](opt: Option[V])(clause: (M, V) => ModifyClause)(implicit ev1: Required[State, Unlimited with Unskipped],
    ev2: RequireShardKey[M, State]): FindAndModifyQuery[M, R] = {

 ...
 */
package reactiverogue.core.json
...
trait JSONGenericHandlers extends GenericHandlers[JsObject, Reads, Writes] {
  import reactiverogue.core.json.BSONFormats._
  ...
  def merge: JsObject = {
    if (!sortOption.isDefined && !hintOption.isDefined && !explainFlag && !snapshotFlag && !commentString.isDefined)
      ...
      Json.obj("$query" -> (queryOption.getOrElse(empty): JsObject)) ++
        sortOption.fold(empty)(o => Json.obj("$orderby" -> o)) ++
        hintOption.fold(empty)(o => Json.obj("$hint" -> o)) ++
        commentString.fold(empty)(o => Json.obj("$comment" -> o)) ++
        option(explainFlag, JsBoolean(true)).fold(empty)(o => Json.obj("$explain" -> o)) ++
        option(snapshotFlag, JsBoolean(true)).fold(empty)(o => Json.obj("$snapshot" -> o))
    }
  }
}
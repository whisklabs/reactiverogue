...
trait MongoMetaRecord[BaseRecord <: MongoRecord[BaseRecord]]
  ...
  def bulkDelete_!!!(implicit ec: ExecutionContext, db: DefaultDB): Future[WriteResult] = {
    db.apply[BSONCollection](collectionName).remove(BSONDocument.empty, WriteConcern.Default, firstMatchOnly = false)
  }

  def count(selector: Option[BSONDocument] = None)(implicit ec: ExecutionContext, db: DefaultDB): Future[Int] = {

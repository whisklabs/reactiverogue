...
trait MongoRecord[MyType <: MongoRecord[MyType]] extends BsonRecord[MyType] with ObjectIdPk[MyType] {
   ...
   */
  def save(concern: WriteConcern)(implicit ec: ExecutionContext, db: DefaultDB): Future[WriteResult] = {
   ...
   */
  def save(implicit ec: ExecutionContext, db: DefaultDB): Future[WriteResult] = save(WriteConcern.Default)

...
trait BsonMetaRecord[BaseRecord <: BsonRecord[BaseRecord]] {
  ...
  private def isField(m: Method) = {
    val ret = !m.isSynthetic && classOf[RecordField[_, _]].isAssignableFrom(m.getReturnType)
    ret
  }
  ...
  def introspect(rec: BaseRecord, methods: Array[Method])(f: (Method, RecordField[_, BaseRecord]) => Any): Unit = {
    ...
    for (v <- realMeth) {
      v.invoke(rec) match {
        case mf: RecordField[_, BaseRecord] =>
  ...
  def fieldDbValue(f: RecordField[_, BaseRecord]): Option[BSONValue] = {
